...
function TradeToTokenDetails({
  ...
  const fetchFromTokenBalance = async () => {
    ...
    try {
      if (tradeToToken?.address === "native") {
        ...
        // console.log("setToTokenBalance", balance);

        setToTokenBalance(balance == "0" ? "0.00" : balance);
      }
    } catch (error) {
      console.error(
        `Error while fetching ${tradeToToken?.symbol} balance`,
        error
      );
    } finally {
  ...
  useEffect(() => {
    if (tradeToToken && address) {
      fetchFromTokenBalance();
    }
    setUpdateBalance(false);
  }, [tradeToToken, updateBalance, address, chainId]);

  const handleClickMax = () => {
    setTradeToTokenInputAmount(toTokenBalance);
    setTradeToInputQuote(true);
    setIsLoadingTopQuote(true);
    setToInputQuote(false);
    setIsLoadingQ2(true);
    setTradeFromInputQuote(false);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setTradeToTokenInputAmount(input);
        ...
        <div className="px-[0.5rem] py-[0.5rem] flex flex-col gap-2">
          <div className="w-full flex flex-row justify-between items-center">
            ...
            {!address ? (
              <div className="flex justify-end items-center gap-1 text-[12px] font-normal dark:text-[#FFFFFF99] text-[#00000099] ">
                Connect wallet to view {tradeToToken?.symbol} balance
              </div>
            ) : (
          ...
          <div className="w-full flex flex-row justify-between items-center !h-[52.5px]">
            ...
            <div className="flex flex-row space-x-2 items-center justify-center">
              {tradeToToken?.logoURI ? (
                <Image
                  src={tradeToToken?.logoURI as string}
                  alt={tradeToToken?.name as string}
                  width={0}
                  height={0}
                  sizes="100vw"
...
export default TradeToTokenDetails;
