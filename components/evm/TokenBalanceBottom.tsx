...
const TokenBalanceBottom = () => {
  ...
  const inputCalculator = () => {
    // normalize values from store (could be strings)
    const apr = Number(activePriceRange); // e.g. 0.1, 0.2, 0.5, 1
    const EPS = 1e-9;

    // FULL RANGE: treat apr ~= 1 as full-range (no price conversion / special logic)
    const isFullRange = Math.abs(apr - 1) < EPS;

    // TOP -> calculate output amount when user changed from-token
    if (lpCalTop) {
      setLpCalBottom(false);
      if (!fromLPTokenInputAmount || !basePrice) return;

      const fromAmount = Number(fromLPTokenInputAmount);
      if (Number.isNaN(fromAmount)) return;
      ...
      if (
        ...
        // full-range fallback or chain/pool not set
        const outputAmount = fromAmount * Number(basePrice);

        setToLPTokenInputAmount(outputAmount.toFixed(18).replace(/\.?0+$/, ""));
        return;
      }

      // Otherwise derive price from apr (e.g. ±10%, ±20%, etc)
      const currentPrice = Number(basePrice);
      const lowerPrice = currentPrice * (1 - apr);
      const upperPrice = currentPrice * (1 + apr);

      // Choose passValue logic that makes sense for your product; here I'll take midpoint
      const passValue = Math.sqrt(lowerPrice * upperPrice);

      const outputAmount = fromAmount * passValue;

      setToLPTokenInputAmount(outputAmount.toFixed(18).replace(/\.?0+$/, ""));
      setActiveValue(null);
    }

    // BOTTOM -> calculate from-token when user changed to-token
    if (lpCalBottom) {
      setLpCalTop(false);
      if (!toLPTokenInputAmount || !basePrice) return;

      const toAmount = Number(toLPTokenInputAmount);
      ...
      if (
        ...
        const inputAmount = toAmount / Number(basePrice);
        setFromLPTokenInputAmount(
      ...
      // Use midpoint price inversed
      const currentPrice = Number(basePrice);
      const lowerPrice = currentPrice * (1 - apr);
      const upperPrice = currentPrice * (1 + apr);
      const passValue = Math.sqrt(lowerPrice * upperPrice);

      const inputAmount = toAmount / passValue;
