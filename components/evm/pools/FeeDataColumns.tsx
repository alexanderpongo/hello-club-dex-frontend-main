...
const formatTokenFromValue = (value: any, deci: number) => {
  ...
  const formatValue = formatUnits(
    BigInt(Math.floor(parseFloat(value?.amount0!))),
    deci
  );
...
const formatTokenToValue = (value: any, deci: number) => {
  ...
  const formatValue = formatUnits(
    BigInt(Math.floor(parseFloat(value?.amount1!))),
    deci
  );
  // return parseFloat(formatValue).toFixed(9);
  return getTrimmedResult(formatValue);
};
...
export const FeeColumns: ColumnDef<LPAdded>[] = [
  {
    accessorKey: "time",
    header: ({ column }) => (
