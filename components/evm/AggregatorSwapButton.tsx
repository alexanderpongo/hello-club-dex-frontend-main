...
import { contractConfig, getDexConfig } from "@/config/blockchain.config";
import { ContractConfigItemType, TokenType } from "@/interfaces/index.i";
import { useConnectModal } from "@rainbow-me/rainbowkit";
import { Loader2 } from "lucide-react";
import Link from "next/link";
import { checkAggregatorApproveAllowance } from "@/service/blockchain.service";
import { toast } from "react-toastify";
import { BASE_CHAIN_CONFIG } from "./TestSwapButton";
import { aggregatorPcsAbi } from "@/blockchain/web3.config";

function AggregatorSwapButton() {
  ...
  const swapHandler = async () => {
    ...
    try {
      ...
      const tokenIn = inputToken! as Address;
      const tokenOut = outputToken! as Address;

      const { routerAbi, swapAggregatorAddress } = getDexConfig(
        aggregatorResult?.dex ?? "pancakeswap",
        chainId ?? 97
      );
      const aggregatorAddress = swapAggregatorAddress;
      const aggregatorAbi = routerAbi ?? aggregatorPcsAbi;
      console.log("aggregatorAddress", aggregatorAddress);
      const amountIn = amountInput;
      const minAmountOut = amountOutMinimum;
      const router = aggregatorResult.router! as Address;
      const isNativeIn = fromToken?.address === "native" ? true : false;
      const isNativeOut = toToken?.address === "native" ? true : false;

      console.log(
        "aggregator swap params ",
        tokenIn,
        tokenOut,
        amountIn,
        minAmountOut,
  ...
  const { swapAggregatorAddress } = getDexConfig(
    aggregatorResult?.dex ?? "pancakeswap",
    chainId ?? 97
  );
  ...
  const handleApprove = async (token: TokenType, amount: string) => {
    ...
    const aggregatorAddress = swapAggregatorAddress as Address;
    console.log("approving token", token, "amount", amount);
  ...
  const fetchFromTokenApprovedAmount = async () => {
    if (fromToken) {
      const fromTokenAllowance = await checkAggregatorApproveAllowance(
        address,
        fromToken?.address as Address,
        fromToken?.chainId,
        setIsLoadingFetchApprovedAmount,
        config,
        swapAggregatorAddress as Address
      );
  ...
  const fetchToTokenApprovedAmount = async () => {
    if (toToken && toToken.address !== "native") {
      const toTokenAllowance = await checkAggregatorApproveAllowance(
        address,
...
export default AggregatorSwapButton;
