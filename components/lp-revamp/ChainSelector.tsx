...
const chains = [
  ...
  },
  {
    id: 1,
    name: "Ethereum",
    icon: "https://etherscan.io/images/svg/brands/ethereum-original-light.svg",
  },
  {
    id: 8453,
    name: "Base",
...
export default function ChainSelector({ noBlackBG }: { noBlackBG?: boolean }) {
  ...
  const {
    setCurrencyA,
    setCurrencyB,
    setCurrencyALPTokenInputAmount,
    setCurrencyBLPTokenInputAmount,
    setFeeTier,
  ...
  return (
    // <Select
    //   value={selectedChain}
    //   onValueChange={(value) => handleSwitchChain(Number(value))}
    //   open={isOpen}
    //   onOpenChange={(open) => {
    //     if (open && !address) {
    //       openConnectModal?.();
    ...
    //   <SelectTrigger
    //     className={`w-full ${
    //       noBlackBG ? "dark:bg-dark" : "dark:bg-black"
    //     } border-[2px] dark:border-[#ffffff14]`}
    //   >
    //     <SelectValue
    //       placeholder={
    //         address ? "Select a chain" : "Connect wallet to select chain"
    //       }
    //     />
    //   </SelectTrigger>
    //   <SelectContent className="bg-white dark:bg-[#1a1a1a] border-[2px] dark:border-[#ffffff14] w-full rounded-[12px] p-2">
    //     <SelectGroup>
    ...
    //           className="flex flex-row w-full items-center gap-2 hover:cursor-pointer hover:bg-[#cecccce9] dark:hover:bg-[#3c3c3ce9]"
    //         >
    ...
    //             />
    //             <p className="inline-flex">{chain.name}</p>
    //           </div>
    //         </SelectItem>
    //       ))}
    //     </SelectGroup>
