...
const LPSlippageSettingDialog = () => {
  const { setlpSlippage, lpSlippage } = useLiquidityPoolStore();

  const [isLastTabActive, setIsLastTabActive] = useState(false);
  const [activeValue, setActiveValue] = useState((lpSlippage / 100).toString());
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <div className="flex justify-end items-center border dark:border-[#FFFFFF1A] dark:bg-[#1A1A1A] text-sm  p-1 group cursor-pointer hover:border-primary rounded-sm">
